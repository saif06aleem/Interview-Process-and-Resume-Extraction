...
class QuizApp:
    def __init__(self, root):
        ...
        self.emotion_model = load_model("C:/Users/USER/Desktop/Project_Main/INTERVIEW_PROJECT_MAIN/my_model.zip")
    ...
    def _record_audio(self, duration):
        ...
        sd.wait()
        save_directory = "C:/Users/USER/Desktop/Project_Main/INTERVIEW_PROJECT_MAIN"
    ...
    def convert_audio_to_text(self, audio_file):
        ...
        try:
            ...
            filename_answer = os.path.join(r'C:/Users/USER/Desktop/Project_Main/INTERVIEW_PROJECT_MAIN',
    ...
    def save_result_to_csv(self, result_text):
        csv_filename = os.path.join("C:/Users/USER/Desktop/Project_Main/INTERVIEW_PROJECT_MAIN", "results.csv")
...
def predcrop():
    ...
    if request.method == 'POST':
        ...
        # Calculate average similarity of the last 3 answers
        last_three_similarities = get_last_three_similarities()
        avg_similarity = sum(last_three_similarities) / len(last_three_similarities)
        avg_similarity = round(avg_similarity, 2)  # Round to 2 decimal places

        # Determine message based on max emotion label
        emotion_message = ""
        if app.max_emotion_label == "Sad":
            emotion_message = "Student should be more confident"
        elif app.max_emotion_label == "Surprise":
            emotion_message = "Student was nervous"
        elif app.max_emotion_label == "Happy":
            emotion_message = "Student was confident enough"

        return render_template('result123.html', max_emotion_label=app.max_emotion_label,
                               max_emotion_count=app.max_emotion_count, avg_similarity=avg_similarity,
                               emotion_message=emotion_message)


def get_last_three_similarities():
    # Read the similarity results from the CSV file
    with open("C:/Users/USER/Desktop/Project_Main/INTERVIEW_PROJECT_MAIN/results.csv", "r") as file:
        lines = file.readlines()

    # Extract the similarity values from the last three lines
    last_three_lines = lines[-3:]
    similarities = [float(line.strip().split(",")[1].split(":")[1][1:-2]) for line in last_three_lines]
...
def upload_file():
    ...
    return redirect(url_for('main_page', message='File uploaded successfully'))
...
def _get_files():
    ...
    if os.path.exists(file_list):
        with open(file_list) as fh:
            data = json.load(fh)
            print('Contents of files.json:', data)  # Debug print to show the contents of files.json
            return data
    return {}
